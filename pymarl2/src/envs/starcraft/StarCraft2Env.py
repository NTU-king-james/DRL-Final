...
class StarCraft2Env(MultiAgentEnv):
    ...
    def __init__(
        ...
        if self.alignment_weight > 0.0:
            self.llm_agent = LLMAgent(model_name="remijang/smac-sft-gemma3", llm_type="ours", verbose=False)
    ...
    def step(self, actions):
        ...
        try:
            ...
            if self.llm_agent is not None and self.alignment_weight > 0.0:
                ...
                self.llm_instruction_history.append(llm_instruction)
    ...
    def get_ally_num_attributes(self):
        """Returns the number of features for each ally unit in the global state."""
        nf_al = 4  # health, cooldown/energy, relative_x, relative_y
        if self.shield_bits_ally > 0:
            nf_al += 1
        if self.unit_type_bits > 0:
    ...
    def get_enemy_num_attributes(self):
        """Returns the number of features for each enemy unit in the global state."""
        nf_en = 3  # health, relative_x, relative_y
        if self.shield_bits_enemy > 0:
            nf_en += 1
        if self.unit_type_bits > 0:
    ...
    def get_llm_inputs(self):
        """Return inputs needed for LLMAgent.act()"""
       
        global_state = self.get_state()
        avail_actions_list = self.get_avail_actions()
        global_state_nl = get_state_NL(self, global_state)
        env_info = self.get_env_info()
        n_agents = env_info["n_agents"]
        return global_state_nl, avail_actions_list, n_agents, env_info
